...
function openPassword(){
  ...
  box.innerHTML='<b>Choose a New Password</b><div class="muted" style="margin:6px 0 10px">You are already signed in, so you can set a new password here.</div><label>New Password</label><input id="cvNewPassword" type="password" autocomplete="new-password" placeholder="New password"><label style="margin-top:8px">Confirm New Password</label><input id="cvNewPassword2" type="password" autocomplete="new-password" placeholder="Type it again"><button class="greenbtn" style="width:100%;margin-top:10px" id="cvSavePassword">Save New Password</button><button class="softbtn" style="width:100%;margin-top:8px" id="cvCancelPassword">Cancel</button>';
...
async function savePassword(){
  const p=document.getElementById('cvNewPassword')?.value||'';
  const p2=document.getElementById('cvNewPassword2')?.value||'';
  ...
  alert('Password changed successfully. You can use the new password to sign in on your computer.');
}
