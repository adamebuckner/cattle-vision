...
function addDogOption(sel){if(!sel||[...sel.options].some(o=>o.value===DOG))return;const o=document.createElement('option');o.value=DOG;o.textContent=DOG;sel.appendChild(o)}
function setupDogOptions(){addDogOption($('aSex'));addDogOption($('rSex'));document.querySelectorAll('select.bulk-sex').forEach(addDogOption)}
function isDog(a){return a&&a.sex===DOG}
function isHorse(a){return a&&a.sex===HORSE}
function isCattle(a){return a&&!isDog(a)&&!isHorse(a)}
function currentPastureStart(a,pasture){const hist=Array.isArray(a.locationHistory)?a.locationHistory:[];for(let i=hist.length-1;i>=0;i--){const h=hist[i];if(String(h.to||'').toLowerCase()===String(pasture||'').toLowerCase()&&h.date)return new Date(h.date)}if(a.created&&String(a.location||'').toLowerCase()===String(pasture||'').toLowerCase())return new Date(a.created);return null}
...
function openGuardianDog(){openAdd();setupDogOptions();$('aSex').value=DOG;$('addTitle').textContent='Add Livestock Guardian Dog'}
...
const origRender=window.render;if(typeof origRender==='function')window.render=async function(){const out=await origRender.apply(this,arguments);setupDogOptions();return out};
...
const origBulkRender=window.renderBulkSorter;if(typeof origBulkRender==='function')window.renderBulkSorter=function(){const out=origBulkRender.apply(this,arguments);setupDogOptions();return out};
setupDogOptions();if(typeof window.render==='function')window.render();
})();