(function setupHerdCollapse(){
  ...
  let collapsed=localStorage.getItem('cv2-herd-collapsed');
  collapsed=collapsed===null?true:collapsed==='1';
  ...
  const toggle=document.createElement('button');toggle.type='button';toggle.className='softbtn';controls.appendChild(toggle);head.appendChild(controls);
  ...
  toggle.addEventListener('click',()=>{collapsed=!collapsed;localStorage.setItem('cv2-herd-collapsed',collapsed?'1':'0');update();if(!collapsed&&typeof window.render==='function')window.render()});
  const search=document.getElementById('search');if(search)search.addEventListener('input',()=>{if(search.value.trim()&&collapsed){collapsed=false;localStorage.setItem('cv2-herd-collapsed','0');update();if(typeof window.render==='function')window.render()}});
